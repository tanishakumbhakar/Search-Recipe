...
function performSearch(event) {
  ...
  $("#search-results-heading").text("Hold On ...");
  $("#results").text("");

  // Send the request
//herokuapp masks URL to prevent cross origin resource sharing
//i and q are declared in the API documentation of recipepuppy
  request = $.ajax({
...
//This jQuery XHR object, or "jqXHR," returned by $.post() implements the Promise interface, giving it all the properties, methods, and behavior of a Promise
  ...
  if (jsonObject.results.length == 0) {
    ...
    jsonObject.results.forEach(
      function(item, index) {
        ...
        if (thumbnail == "") { thumbnail = "images/nophoto.jpg"; }
